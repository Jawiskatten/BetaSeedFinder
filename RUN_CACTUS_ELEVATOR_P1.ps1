param(
    ...
    [ValidateRange(100000,100000000)][UInt64]$ScoutChunk = 5000000,
    [ValidateRange(256,32768)][int]$Batch = 8192,
    [ValidateSet(64,128,256)][int]$TerrainThreads = 64,
    [ValidateRange(1,62)][int]$MinLift = 8,
    [switch]$Resume
)
...
# Pinned dependency snapshot containing GPU scout + fast oracle + full client oracle.
$BranchRef = '76c5a2185ca8b13524618629c5b3ed67fc5ce81c'
...
$build = Join-Path $ProjectRoot 'build\cactus-elevator-p1'
$srcDir = Join-Path $build 'source'
$genDir = Join-Path $build 'generated-full-r4'
...
$scoutSource = Join-Path $srcDir 'CactusElevatorScoutP1.cpp'
$highestSource = Join-Path $srcDir 'HighestPillarSpawnGpuFinder.cpp'
$prefilterSource = Join-Path $oracleSrcDir 'Beta173CactusElevatorFastPrefilter.java'
$oracleSource = Join-Path $oracleSrcDir 'Beta173CactusElevatorOracle.java'
Invoke-WebRequest -UseBasicParsing "$RawBase/native/cactus_elevator/CactusElevatorScoutP1.cpp" -OutFile $scoutSource
...
Invoke-WebRequest -UseBasicParsing "$RawBase/tools/Beta173CactusElevatorFastPrefilter.java" -OutFile $prefilterSource
Invoke-WebRequest -UseBasicParsing "$RawBase/tools/Beta173CactusElevatorOracle.java" -OutFile $oracleSource

# We need radius-4 density because cactus validity checks the four horizontal
# neighbors around origin. Copy generated headers locally, then apply the P6
# vanilla chunk-local climate-coordinate correction without shrinking radius.
...
$scoutExe = Join-Path $build 'CactusElevatorScoutP1_AMD.exe'
Write-Host "Compiling cactus-elevator GPU scout for $archKey..."
& $hipcc -O3 -std=c++17 -x hip @archArgs '-DSKYBLOCK_COARSE_API_MODERN=1' `
    "-I$srcDir" "-I$genDir" "-I$nativeSourceDir" $scoutSource -o $scoutExe | Out-Host
if ($LASTEXITCODE -ne 0) { throw 'Cactus-elevator AMD compilation failed.' }
...
Write-Host 'Compiling four-chunk cactus prefilter + authoritative client-startup oracle...'
& $javac -source 8 -target 8 -encoding UTF-8 -cp $betaBin -d $classes $prefilterSource $oracleSource
if ($LASTEXITCODE -ne 0) { throw 'Cactus-elevator Java verifier compilation failed.' }
...
$outputRoot = Join-Path $ProjectRoot 'out\cactus_elevator_p1'
New-Item -ItemType Directory -Force -Path $outputRoot | Out-Null
$lastRun = Join-Path $outputRoot 'LAST_RUN.txt'
if ($Resume) {
    if (-not (Test-Path $lastRun -PathType Leaf)) { throw 'No cactus-elevator LAST_RUN.txt exists.' }
    $output = (Get-Content $lastRun -Raw).Trim()
    if (-not (Test-Path $output -PathType Container)) { throw "Cactus-elevator run directory missing: $output" }
} else {
...
$output = [IO.Path]::GetFullPath($output)
$scoutDir = Join-Path $output 'gpu_geometry_candidates'
...
$fastBest = Join-Path $output 'BEST_FAST_CA​​CTUS_ELEVATOR.txt'
$fullBest = Join-Path $output 'BEST_AUTHORITATIVE_CACTUS_ELEVATOR.txt'
...
if ($Resume) {
    ...
    if ($kv.ContainsKey('MIN_LIFT')) { $MinLift=[int]$kv['MIN_LIFT'] }
} else {
    $completed=[UInt64]0
...
Write-Host ''
Write-Host '=================================================================='
Write-Host ' BETA 1.7.3 CACTUS SPAWN ELEVATOR P1'
Write-Host '=================================================================='
Write-Host 'Mechanism: spawn selector accepts desert sand Y63 because Y64 is air.'
Write-Host 'A population cactus at (0,64,0) can bridge the player into an overhead terrain mass.'
Write-Host 'Beta preparePlayerToSpawn then increments Y one block at a time until the player no longer collides.'
Write-Host "GPU requires a hypothetical 2/3-high cactus to predict lift >= $MinLift."
Write-Host 'FAST JAVA runs exact four-chunk population and requires a real origin cactus stack.'
Write-Host 'FULL JAVA reproduces the exact 17x17 client Building-terrain startup.'
Write-Host 'Best-state files persist across every scout chunk and resume.'
...
while($completed -lt $Count) {
    ...
    Write-Host "--- CACTUS GPU GEOMETRY SCOUT start=$index count=$n ---"
    & $scoutExe --output $scoutDir --count $n.ToString($invariant) --start-index $index.ToString($invariant) `
        ...
        --terrain-threads $TerrainThreads --min-lift $MinLift --progress-ms 1000
    if ($LASTEXITCODE -ne 0) { throw "Cactus-elevator GPU scout failed at start=$index" }
    ...
    Write-Host "GPU cactus-bridge geometry candidates: $candidateCount"

    if ($candidateCount -gt 0) {
        Write-Host 'Running exact four-chunk population prefilter...'
        & $java '-Xmx2G' '-Djava.awt.headless=true' -cp $cp net.minecraft.src.Beta173CactusElevatorFastPrefilter `
            --input $candidateFile --output $prefilterDir --best-state $fastBest --min-lift $MinLift --progress-every 25
        if ($LASTEXITCODE -ne 0) { throw "Fast cactus-elevator prefilter failed at start=$index" }
        ...
        Write-Host "FULL client-startup cactus-elevator shortlist in chunk: $shortlistCount"

        if ($shortlistCount -gt 0) {
            Write-Host 'Running authoritative Beta CLIENT 17x17 startup oracle...'
            & $java '-Xmx4G' '-Djava.awt.headless=true' -cp $cp net.minecraft.src.Beta173CactusElevatorOracle `
                --input $shortlistFile --output $verifyDir --best-state $fullBest --min-lift $MinLift --progress-every 1
            if ($LASTEXITCODE -ne 0) { throw "Full cactus-elevator oracle failed at start=$index" }
...
if (Test-Path $fastBest) { Write-Host 'BEST FOUR-CHUNK RESULT:'; Get-Content $fastBest }
if (Test-Path $fullBest) { Write-Host 'BEST AUTHORITATIVE RESULT:'; Get-Content $fullBest }
$summary = Join-Path $verifyDir 'SUMMARY.txt'
if (Test-Path $summary) { Get-Content $summary }
$top = Join-Path $verifyDir 'top_cactus_elevators.csv'
if (Test-Path $top) {
    Write-Host ''
    Write-Host 'TOP AUTHORITATIVE CACTUS ELEVATORS:'
    Import-Csv $top | Select-Object -First 25 rank,seed,lift,cactus_height,post_feet_y,post_support_y | Format-Table -AutoSize
}
