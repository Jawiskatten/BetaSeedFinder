...
# HighestPillar include through our include path and rename its standalone main because
# this translation unit embeds the visual implementation as a library.
...
$visualText = $visualText.Replace('int main(int argc, char** argv) {', 'int visual_rank_embedded_main(int argc, char** argv) {')
[IO.File]::WriteAllText($visual, $visualText, [Text.UTF8Encoding]::new($false))

$skyText = [IO.File]::ReadAllText($sky)
# The visual main is already renamed above, so remove the outer main macro wrapper to avoid
# it being overwritten by the visual file's own HighestPillar include macro.
$skyText = $skyText.Replace("#define main floating_island_spawn_visual_embedded_main`r`n", '')
$skyText = $skyText.Replace("#define main floating_island_spawn_visual_embedded_main`n", '')
$skyText = $skyText.Replace("#undef main`r`n`r`n#include <deque>", "#include <deque>")
$skyText = $skyText.Replace("#undef main`n`n#include <deque>", "#include <deque>")
...
# Save every row rather than limiting the file named all_skyblock_prefilter_ranked.csv to 10k.
