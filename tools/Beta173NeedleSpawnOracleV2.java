...
/**
 * Exact Beta 1.7.3 verifier for a genuinely freestanding 1x1 sand needle.
 *
 * V1 only required the eight neighbors to be air at the TOP block and then
 * ranked pillarDepth. That allowed a scout candidate whose top looked isolated
 * but whose shaft rejoined a mountain a few blocks lower. V2 makes the 1x1
 * shaft depth a hard gate.
 */
public final class Beta173NeedleSpawnOracleV2 {
    ...
    private static final class Result {
        ...
        int needleDepth=0,r2=-1,r4=-1,clearR2=0,clearR4=0;
    ...
    private static World fresh(long seed){return new World(new MemorySaveHandler(),"needle-spawn-oracle-v2",seed,(WorldProvider)null);}
    ...
    private static int freestandingNeedleDepth(World w,int topY){
        int d=0;
        for(int y=topY;y>=0;--y){
            if(!hasCollision(w,0,y,0))break;
            if(!allEightAir(w,y))break;
            ++d;
        }
        return d;
    }

    private static Result verify(Candidate c,int minDrop,int minTop,int minNeedleDepth){
        ...
        try{
            ...
            r.needleDepth=freestandingNeedleDepth(w,r.finalTop);r.clearR2=clearAtTop(w,r.finalTop,2);r.clearR4=clearAtTop(w,r.finalTop,4);r.playerFeet=actualFeet(w);

            if(r.minCard<minDrop){r.status="CARDINAL_DROP_TOO_SMALL";return r;}
            if(r.needleDepth<minNeedleDepth){r.status="NEEDLE_REJOINS_TERRAIN_TOO_EARLY";return r;}
            // A valid spawn needle should put the player directly one block above
            // the final sand cap; this also rejects odd population/collision cases.
            if(r.playerFeet!=r.finalTop+1){r.status="PLAYER_NOT_ON_NEEDLE_TOP";return r;}

            r.hit=true;r.status="NEEDLE_HIT";
            r.score=(long)r.minCard*1000000000000L+(long)r.needleDepth*1000000000L+(long)r.finalTop*1000000L+(long)Math.max(0,Math.min(999,r.minEight))*1000L+(long)Math.max(0,Math.min(999,r.r2));
    ...
    private static void writeCsv(File f,List<Result> rows)throws Exception{
        ...
        out.println("rank,seed,sequence_index,status,hit,exact_score,pre_surface_y,final_top_y,player_feet_y,north_drop,south_drop,east_drop,west_drop,min_cardinal_drop,min_eight_drop,needle_depth,r2_min_drop,r4_min_drop,clear_r2,clear_r4,scout_top_y,scout_min_positive_drop,scout_quadrant_depth");
        for(int i=0;i<rows.size();++i){Result r=rows.get(i);out.println((i+1)+","+r.c.seed+","+r.c.seq+","+r.status+","+(r.hit?1:0)+","+r.score+","+r.preSurface+","+r.finalTop+","+r.playerFeet+","+r.north+","+r.south+","+r.east+","+r.west+","+r.minCard+","+r.minEight+","+r.needleDepth+","+r.r2+","+r.r4+","+r.clearR2+","+r.clearR4+","+r.c.scoutTop+","+r.c.scoutMinDrop+","+r.c.scoutDepth);}
        out.close();
    }
    public static void main(String[]args)throws Exception{
        File input=null,output=null;int minDrop=10,minTop=70,minNeedleDepth=10;
        for(int i=0;i<args.length;++i){if("--input".equals(args[i]))input=new File(args[++i]);else if("--output".equals(args[i]))output=new File(args[++i]);else if("--min-drop".equals(args[i]))minDrop=Integer.parseInt(args[++i]);else if("--min-top-y".equals(args[i]))minTop=Integer.parseInt(args[++i]);else if("--min-needle-depth".equals(args[i]))minNeedleDepth=Integer.parseInt(args[++i]);else throw new IllegalArgumentException("unknown arg "+args[i]);}
        ...
        for(Candidate c:cs){Result r=verify(c,minDrop,minTop,minNeedleDepth);rows.add(r);Integer old=rejects.get(r.status);rejects.put(r.status,old==null?1:old+1);if(r.hit){++hits;System.out.println("EXACT NEEDLE HIT seed="+c.seed+" topY="+r.finalTop+" minCard="+r.minCard+" N/S/E/W="+r.north+"/"+r.south+"/"+r.east+"/"+r.west+" needleDepth="+r.needleDepth+" r2="+r.r2+" r4="+r.r4);}++done;if(done%25==0||done==cs.size()){double sec=(System.nanoTime()-started)/1e9;System.out.println("needle-v2 "+done+"/"+cs.size()+" rate="+String.format(Locale.ROOT,"%.2f",done/Math.max(sec,0.001))+"/s hits="+hits);}}
        ...
        writeCsv(output,rows);System.out.println("NEEDLE V2 DONE rows="+done+" hits="+hits+" OUTPUT="+output.getAbsolutePath());
        ...
        if(!rows.isEmpty()&&rows.get(0).hit){Result b=rows.get(0);System.out.println("BEST EXACT NEEDLE seed="+b.c.seed+" topY="+b.finalTop+" minCard="+b.minCard+" needleDepth="+b.needleDepth+" r2="+b.r2+" r4="+b.r4);}
    }
}
