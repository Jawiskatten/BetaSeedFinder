...
Write-Host ("HighestPillarSpawn P1 preflight: exact Beta origin sand gate, actual player collision push, and isolated 1x1 support top; radius={0}." -f $Radius)
...
if ($api -ne 'modern') { throw 'HighestPillarSpawn P1 requires the current modern BetaSeedFinder GPU headers.' }
$apiDefine = '-DSKYBLOCK_COARSE_API_MODERN=1'
$nativeDir = Join-Path $ProjectRoot ("build\highest-pillar-spawn-p1-full-r{0}\native" -f $Radius)
New-Item -ItemType Directory -Force -Path $nativeDir | Out-Null
$source = Join-Path $ProjectRoot 'native\highest_pillar_spawn\HighestPillarSpawnGpuFinder.cpp'
if (-not (Test-Path $source -PathType Leaf)) { throw "HighestPillarSpawn source missing: $source" }
$worker = Join-Path $nativeDir ("HighestPillarSpawnGpuFinderAMD_P1_full_r{0}.exe" -f $Radius)
$sigFile = "$worker.signature.txt"
$archKey = ($arches -join ',')
$sig = Get-NativeSignature $source $nativeSourceDir "AMD|$archKey|$api" ("HighestPillarSpawnP1|full|r$Radius")
$old = if (Test-Path $sigFile) { Get-Content $sigFile -Raw } else { '' }
if (-not (Test-Path $worker) -or $old -ne $sig) {
    Write-Host "Compiling HighestPillarSpawn P1 AMD GPU finder for $archKey..."
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        throw 'HighestPillarSpawn P1 AMD compilation failed.'
    }
    Set-Content $sigFile $sig -Encoding UTF8 -NoNewline
} else {
    Write-Host "Using cached HighestPillarSpawn P1 full/r$Radius AMD worker."
}
...
if ($LASTEXITCODE -ne 0) {
    Remove-Item $worker -Force -ErrorAction SilentlyContinue
    throw 'HighestPillarSpawn P1 GPU self-test failed.'
}
if ($SelfTest) { exit 0 }

$outputRoot = Join-Path $ProjectRoot 'out\highest_pillar_spawn_p1'
...
Set-Content (Join-Path $output 'HOW_TO_RESUME.txt') 'Run RESUME_HIGHEST_PILLAR_SPAWN_P1_LAST_RUN.bat. It preserves the unique48 sequence and leaderboards.' -Encoding UTF8
...
try {
    ...
    Write-Host ''
    Write-Host 'Hard gate: X=0,Z=0 is a valid Beta sand spawn and the final support block is the only solid block in its 3x3 at that Y.'
    Write-Host 'Primary ranking: actual player feet Y after Beta-style upward collision pushing.'
    Write-Host 'Tie-breaks: consecutive 1x1 pillar depth, then local vertical drop.'
    Write-Host 'Population caves/lakes are not applied in the GPU scout, so verify record seeds in Beta 1.7.3.'
    Write-Host "Saving this run to: $output"
    & $worker @workerArgs
    $workerExitCode = $LASTEXITCODE
    if ($workerExitCode -ne 0) { throw "HighestPillarSpawn P1 AMD search exited with code $workerExitCode." }
