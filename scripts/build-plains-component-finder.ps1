...
if (-not (Test-Path $source -PathType Leaf)) {
    throw "Plains component source not found after generation: $source"
}

$outputDir = Join-Path $root 'build\native\amd'
$output = Join-Path $outputDir 'PlainsComponentFinder.exe'
$log = Join-Path $outputDir 'plains-component-build.log'
...
function Import-VisualStudioEnvironment {
    ...
    $tempCommand = Join-Path ([IO.Path]::GetTempPath()) (
        'PlainsComponent-vsenv-' + [Guid]::NewGuid().ToString('N') + '.cmd'
    )
    @(
        '@echo off'
...
if ($exitCode -ne 0) {
    throw "PlainsComponentFinder build failed with exit code $exitCode. See $log"
}
