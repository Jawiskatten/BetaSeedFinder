...
if ($text.Contains('TU4_WATER_P13_SCREEN_RECALL_AUDIT')) {
    Write-Host 'TU4 Water P13 screen recall audit is already applied.' -ForegroundColor Green
    exit 0
}
if (-not $text.Contains('TU4_WATER_P12_DIRECT_SCREEN')) {
    throw 'P13 requires the P12c direct exact-density screen first.'
}
if (-not $text.Contains('TU4_WATER_P8_BATCHED_EXACT')) {
    throw 'P13 expects the P8 batched exact evaluator underneath P12c.'
}

$backupPath = $sourcePath + '.p12c-before-water-p13.bak'
if (-not (Test-Path $backupPath -PathType Leaf)) {
    Copy-Item -LiteralPath $sourcePath -Destination $backupPath
}

# P13 is a diagnostic mode, not a new search heuristic. Normal search behavior
# remains P12c: 24 P6 finalists -> 16x16 full-density screen -> 8 full exacts.
# With --audit-screen-batches N, P13 additionally full-exacts all 24 finalists
# for N batches and measures whether P12c's chosen 8 contained the true best-of-24.
# The audit returns only the chosen-8 results to the normal record loop so normal
# P12c routing semantics remain visible while ground truth is collected privately.

$markerPos = $text.IndexOf('// TU4_WATER_P12_DIRECT_SCREEN')
if ($markerPos -lt 0) { throw 'Could not locate P12c marker.' }
$marker = @'
// TU4_WATER_P13_SCREEN_RECALL_AUDIT
// Optional audit mode full-exacts all P6 finalists and measures P12c 8/24 recall.
// Normal (non-audit) P12c throughput/routing is unchanged.
'@
...
# -------------------------------------------------------------------------
# Audit statistics/helpers before runExactBatch.
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# Inject the ground-truth audit after P12c has selected its chosen 8 but before
# it compacts/rebuilds those seeds. At this point device/host state arrays still
# correspond to all original finalists from the sparse-screen pass, so audit
# can launch the full 201x201 grid for all n without rebuilding/uploading states.
# -------------------------------------------------------------------------
...
$auditBranch = @'
    if (gP13AuditScreen && n > 1) {
        // Ground truth: full-exact ALL pre-finalists using the untouched original
        // device states from the screen pass. This is diagnostic-only work.
        ...
        // Preserve normal P12c routing semantics: only chosen-8 exact results are
        // exposed to the normal record loop even though audit knows all 24.
...
# -------------------------------------------------------------------------
# Main: enable audit and auto-limit to exactly N scout batches unless the user
# explicitly supplied an even smaller --max-attempts. Print final histogram.
# -------------------------------------------------------------------------
...
Write-Host 'Applied TU4 Water P13 screen-recall audit.' -ForegroundColor Green
Write-Host 'Normal search remains P12c (8/24 full exact) and should keep P12c speed.'
Write-Host 'Audit mode full-exacts all 24 and measures whether P12c chose the true best-of-24.'
Write-Host 'Reports chosen8 recall, pure screen top-1/3/6/8 recall, miss regret, tail recall, and rank histogram.'
Write-Host 'Recommended diagnostic: -AuditScreenBatches 2000 -Batch 131072 -TopExact 24.'
