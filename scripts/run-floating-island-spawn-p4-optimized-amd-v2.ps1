...
# Produce the proven full-r4 generated headers, then derive a one-X/Z-column
# specialization for stage 1. Vertical math stays exact and full-height because
# desert sand can occur above beach level; only the enormous unused X/Z window
# is removed from the common path.
...
function Compile-P4Worker(
    ...
    if (-not (Test-Path $worker) -or $old -ne $sig) {
        ...
        & $hipcc -O3 -std=c++17 -x hip @archArgs $apiDefine "-I$GeneratedInclude" "-I$nativeSourceDir" $Source -o $worker 2>&1 | Out-Host
        if ($LASTEXITCODE -ne 0) {
            Remove-Item $worker -Force -ErrorAction SilentlyContinue
            throw "Compilation failed: $ExeName"
        }
        [System.IO.File]::WriteAllText($sigFile, $sig, [System.Text.Encoding]::UTF8)
    } else {
...
$scoutWorker = Compile-P4Worker $scoutSource $originGenerated 'FloatingIslandSpawnScoutP4_AMD.exe' 'P4|origin-only|size1|v2'
$r4Worker = Compile-P4Worker $verifySource $fullR4Generated 'FloatingIslandSpawnVerifyP4_r4_AMD.exe' 'P4|verify|r4|ground-shortcut|bulkcopy|v2'
...
$script:r8WorkerCache = $null
$script:r12WorkerCache = $null
function Get-P4Verifier([int]$Radius) {
    if ($Radius -eq 4) { return $r4Worker }
    if ($Radius -eq 8 -and $null -ne $script:r8WorkerCache) { return $script:r8WorkerCache }
    if ($Radius -eq 12 -and $null -ne $script:r12WorkerCache) { return $script:r12WorkerCache }
    ...
    $worker = Compile-P4Worker $verifySource $generated ("FloatingIslandSpawnVerifyP4_r{0}_AMD.exe" -f $Radius) ("P4|verify|r{0}|ground-shortcut|bulkcopy|v2" -f $Radius)
    Write-Host ("Running P4 r{0} verifier self-test..." -f $Radius)
    & $worker --self-test | Out-Host
    if ($LASTEXITCODE -ne 0) { throw ("P4 r{0} verifier self-test failed." -f $Radius) }
    if ($Radius -eq 8) { $script:r8WorkerCache = $worker } else { $script:r12WorkerCache = $worker }
    return $worker
}

function Has-CandidateRows([string]$Path) {
    if (-not (Test-Path $Path -PathType Leaf)) { return $false }
    return @(Get-Content -LiteralPath $Path -TotalCount 2).Count -ge 2
}
...
if ($Resume) {
    ...
    $rawLast = Get-Content -LiteralPath $lastRunFile -Raw
    $output = [regex]::Replace($rawLast, '^[\uFEFF\u200B\s]+|[\s]+$', '')
...
function Save-P4Checkpoint {
    $text = @(
        'VERSION=FloatingIslandSpawnP4OptimizedV2',
...
Write-Host 'Stage 3: only non-ground components that hit the window edge expand to r8, then r12.'
...
while ($completed -lt $Count) {
    ...
    [UInt64]$chunkLimit = [UInt64]$ScoutChunk
    [UInt64]$chunkCount = if ($left -lt $chunkLimit) { $left } else { $chunkLimit }
    ...
    if (Has-CandidateRows $candidateFile) {
        ...
        Write-Host 'No scout candidates in this chunk; skipping every full-window terrain pass.'
    }
