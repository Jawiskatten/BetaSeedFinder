...
Write-Host ("CursedSpawnOrigin P1 preflight: exact Beta 1.7.3 sand gate at X=0,Z=0; fixed-origin danger scoring, radius={0}." -f $Radius)
...
if ($api -ne 'modern') { throw 'CursedSpawnOrigin P1 requires the current modern BetaSeedFinder GPU headers.' }
$apiDefine = '-DSKYBLOCK_COARSE_API_MODERN=1'
$nativeDir = Join-Path $ProjectRoot ("build\cursed-spawn-origin-p1-full-r{0}\native" -f $Radius)
...
$worker = Join-Path $nativeDir ("CursedSpawnOriginGpuFinderAMD_P1_full_r{0}.exe" -f $Radius)
$sigFile = "$worker.signature.txt"
$archKey = ($arches -join ',')
$sig = Get-NativeSignature $source $nativeSourceDir "AMD|$archKey|$api" ("CursedSpawnOriginP1|full|r$Radius")
$old = if (Test-Path $sigFile) { Get-Content $sigFile -Raw } else { '' }
if (-not (Test-Path $worker) -or $old -ne $sig) {
    Write-Host "Compiling CursedSpawnOrigin P1 AMD GPU finder for $archKey..."
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        throw 'CursedSpawnOrigin P1 AMD compilation failed.'
    }
    Set-Content $sigFile $sig -Encoding UTF8 -NoNewline
} else {
    Write-Host "Using cached CursedSpawnOrigin P1 full/r$Radius AMD worker."
}
...
if ($LASTEXITCODE -ne 0) {
    Remove-Item $worker -Force -ErrorAction SilentlyContinue
    throw 'CursedSpawnOrigin P1 GPU self-test failed.'
}
if ($SelfTest) { exit 0 }

$outputRoot = Join-Path $ProjectRoot 'out\cursed_spawn_origin_p1'
...
Set-Content (Join-Path $output 'HOW_TO_RESUME.txt') 'Run RESUME_CURSED_SPAWN_ORIGIN_P1_LAST_RUN.bat. It preserves the unique48 sequence and merges the existing leaderboards.' -Encoding UTF8
...
try {
    ...
    Write-Host ''
    Write-Host 'Every saved candidate is spawn-valid sand at exactly world X=0,Z=0 in the unpopulated Beta terrain.'
    Write-Host 'Scoring: smallest safe area, surrounding water, immediate cliffs, overhead/floating terrain, and combinations.'
    Write-Host "Saving this run to: $output"
    & $worker @workerArgs
    $workerExitCode = $LASTEXITCODE
    if ($workerExitCode -ne 0) { throw "CursedSpawnOrigin P1 AMD search exited with code $workerExitCode." }
