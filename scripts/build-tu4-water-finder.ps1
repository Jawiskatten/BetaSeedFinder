...
$source = Join-Path $root 'native\src\tu4_water_finder.cpp'
if (-not (Test-Path $source -PathType Leaf)) {
    throw "TU4 water finder source not found: $source"
}

$outputDir = Join-Path $root 'build\native\amd'
$output = Join-Path $outputDir 'TU4WaterFinder.exe'
$log = Join-Path $outputDir 'tu4-water-build.log'
...
function Import-VisualStudioEnvironment {
    ...
    $tempCommand = Join-Path ([IO.Path]::GetTempPath()) (
        'TU4Water-vsenv-' + [Guid]::NewGuid().ToString('N') + '.cmd'
    )
    @(
        '@echo off'
...
if ($exitCode -ne 0) {
    throw "TU4 Water Finder build failed with exit code $exitCode. See $log"
}
